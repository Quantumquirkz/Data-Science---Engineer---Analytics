...
from projects.semiconductor_process_variation_analyzer.src.config import SPEC
from projects.semiconductor_process_variation_analyzer.src.pipeline import run_semiconductor_process_variation_analyzer_pipeline


@lru_cache(maxsize=1)
def _load_pipeline(n_samples: int):
    return run_semiconductor_process_variation_analyzer_pipeline(PROJECT_ROOT, n_samples=int(n_samples))
