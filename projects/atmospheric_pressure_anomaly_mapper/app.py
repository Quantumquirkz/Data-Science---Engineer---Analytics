...
from projects.atmospheric_pressure_anomaly_mapper.src.config import SPEC
from projects.atmospheric_pressure_anomaly_mapper.src.pipeline import run_atmospheric_pressure_anomaly_mapper_pipeline


@lru_cache(maxsize=1)
def _load_pipeline(n_samples: int):
    return run_atmospheric_pressure_anomaly_mapper_pipeline(PROJECT_ROOT, n_samples=int(n_samples))
