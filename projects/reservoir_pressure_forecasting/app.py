...
from projects.reservoir_pressure_forecasting.src.config import SPEC
from projects.reservoir_pressure_forecasting.src.pipeline import run_reservoir_pressure_forecasting_pipeline


@lru_cache(maxsize=1)
def _load_pipeline(n_samples: int):
    return run_reservoir_pressure_forecasting_pipeline(PROJECT_ROOT, n_samples=int(n_samples))
