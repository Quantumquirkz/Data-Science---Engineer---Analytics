...
In simple terms, this level helps you version runs, metrics, and models without losing comparability.
...
Complete or skim ml_engineering:L05 first, then return here if a word feels unfamiliar.
...
- **Run lineage**: explain what this means before you try to automate it.
- **Model registry semantics**: identify where it appears in a table, notebook, model, system, or equation.
- **Promotion criteria**: describe how it can mislead you if you ignore it.
...
Imagine a trained model inside an app. You need to know where the package boundary is, how to call it, and how promotion criteria affects repeated runs.
...
- I can say what `p068` / `recommender_system_for_scientific_papers` is trying to teach.
- I can identify the input, output, and one hidden assumption.
- I can explain `Run lineage` using a small example.
- I can explain `Model registry semantics` using the project files or notebook.
- I can explain why `Promotion criteria` matters.
- I can write one limitation without weakening the value of the project.

## Mission Anchor

- Project ID: `p068`
- Project slug: `recommender_system_for_scientific_papers`
- Mission ID: `p068-mle-l06-m01`
- Central notebook: [`notebooks/tracks/ml_engineering/L06/ml_engineering_l06_experiment_tracking_and_registry.ipynb`](../../../../../notebooks/tracks/ml_engineering/L06/ml_engineering_l06_experiment_tracking_and_registry.ipynb)
- Project notebook: `projects/recommender_system_for_scientific_papers/notebooks/recommender_system_for_scientific_papers.ipynb`
- Run command:

```bash
PYTHONPATH=src:. uv run python -m projects.recommender_system_for_scientific_papers.src.pipeline
```
...
The next level is ml_engineering:L07, where this idea becomes more technical.
