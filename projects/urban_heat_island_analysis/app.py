...
from projects.urban_heat_island_analysis.src.config import SPEC
from projects.urban_heat_island_analysis.src.pipeline import run_urban_heat_island_analysis_pipeline


@lru_cache(maxsize=1)
def _load_pipeline(n_samples: int):
    return run_urban_heat_island_analysis_pipeline(PROJECT_ROOT, n_samples=int(n_samples))
