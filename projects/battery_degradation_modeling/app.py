...
from projects.battery_degradation_modeling.src.config import SPEC
from projects.battery_degradation_modeling.src.pipeline import run_battery_degradation_modeling_pipeline


@lru_cache(maxsize=1)
def _load_pipeline(n_samples: int):
    return run_battery_degradation_modeling_pipeline(PROJECT_ROOT, n_samples=int(n_samples))
