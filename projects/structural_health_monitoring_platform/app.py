...
from projects.structural_health_monitoring_platform.src.config import SPEC
from projects.structural_health_monitoring_platform.src.pipeline import run_structural_health_monitoring_platform_pipeline


@lru_cache(maxsize=1)
def _load_pipeline(n_samples: int):
    return run_structural_health_monitoring_platform_pipeline(PROJECT_ROOT, n_samples=int(n_samples))
