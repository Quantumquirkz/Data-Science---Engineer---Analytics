...
from projects.online_experiment_bayesian_analyzer.src.config import SPEC
from projects.online_experiment_bayesian_analyzer.src.pipeline import run_online_experiment_bayesian_analyzer_pipeline


@lru_cache(maxsize=1)
def _load_pipeline(n_samples: int):
    return run_online_experiment_bayesian_analyzer_pipeline(PROJECT_ROOT, n_samples=int(n_samples))
