...
from projects.magnetic_field_sensor_reconstruction.src.config import SPEC
from projects.magnetic_field_sensor_reconstruction.src.pipeline import run_magnetic_field_sensor_reconstruction_pipeline


@lru_cache(maxsize=1)
def _load_pipeline(n_samples: int):
    return run_magnetic_field_sensor_reconstruction_pipeline(PROJECT_ROOT, n_samples=int(n_samples))
