...
from projects.dynamic_pricing_under_uncertainty.src.config import SPEC
from projects.dynamic_pricing_under_uncertainty.src.pipeline import run_dynamic_pricing_under_uncertainty_pipeline


@lru_cache(maxsize=1)
def _load_pipeline(n_samples: int):
    return run_dynamic_pricing_under_uncertainty_pipeline(PROJECT_ROOT, n_samples=int(n_samples))
