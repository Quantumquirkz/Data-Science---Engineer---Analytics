...
@lru_cache(maxsize=1)
def _load_pipeline():
    return run_solar_irradiance_pipeline(
...
def run_demo(horizon: int, max_points: int):
    ...
    cfg = DatasetConfig()
    summary = (
        "### Solar irradiance forecasting (Open-Meteo)\n\n"
        f"- Location: `{cfg.latitude}, {cfg.longitude}` (UTC hourly)\n"
        ...
        f"- Best mean RMSE model: `{artifacts.model_artifacts.best_point_model_name}`\n"
    )
    h = int(horizon)
    n = int(max_points)
...
with gr.Blocks(title="Solar Irradiance Forecasting") as demo:
    gr.Markdown(
        "# Solar irradiance forecasting\n\n"
        "GHI forecasts from Open-Meteo: baselines, Ridge, HGBR, LightGBM quantiles."
    )
    horizon = gr.Dropdown(choices=[1, 3, 6], value=1, label="Forecast horizon (hours)")
    max_points = gr.Slider(100, 2000, value=500, step=50, label="Points in forecast plot")
    ...
    residual_plot = gr.Plot(label="Residuals by hour (UTC)")
    rmse_plot = gr.Plot(label="RMSE heatmap")
    run_button.click(
        fn=run_demo,
