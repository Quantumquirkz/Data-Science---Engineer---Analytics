# Data Analytics L09: Analytics Engineering And Governance
...
In simple terms, this level helps you unify metrics definitions, data contracts, and governance checks.
...
Complete or skim data_analytics:L08 first, then return here if a word feels unfamiliar.
...
- **Metric lineage**: explain what this means before you try to automate it.
- **Access policy**: identify where it appears in a table, notebook, model, system, or equation.
- **Backfill risk**: describe how it can mislead you if you ignore it.
...
Imagine a table with one row per day. Before making a chart, ask what each row means, what metric is being counted, and whether backfill risk could change the interpretation.
...
- I can say what `p074` / `satellite_image_change_detection` is trying to teach.
- I can identify the input, output, and one hidden assumption.
- I can explain `Metric lineage` using a small example.
- I can explain `Access policy` using the project files or notebook.
- I can explain why `Backfill risk` matters.
- I can write one limitation without weakening the value of the project.

## Mission Anchor

- Project ID: `p074`
- Project slug: `satellite_image_change_detection`
- Mission ID: `p074-da-l09-m01`
- Central notebook: [`notebooks/tracks/data_analytics/L09/data_analytics_l09_analytics_engineering_and_governance.ipynb`](../../../../../notebooks/tracks/data_analytics/L09/data_analytics_l09_analytics_engineering_and_governance.ipynb)
- Project notebook: `projects/satellite_image_change_detection/notebooks/satellite_image_change_detection.ipynb`
- Run command:

```bash
PYTHONPATH=src:. uv run python -m projects.satellite_image_change_detection.src.pipeline
```
...
The next level is data_analytics:L10, where this idea becomes more technical.
