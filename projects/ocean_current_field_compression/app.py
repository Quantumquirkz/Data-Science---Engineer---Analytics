...
from projects.ocean_current_field_compression.src.config import SPEC
from projects.ocean_current_field_compression.src.pipeline import run_ocean_current_field_compression_pipeline


@lru_cache(maxsize=1)
def _load_pipeline(n_samples: int):
    return run_ocean_current_field_compression_pipeline(PROJECT_ROOT, n_samples=int(n_samples))
