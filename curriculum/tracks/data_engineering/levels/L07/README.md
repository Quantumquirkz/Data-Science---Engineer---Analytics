...
In simple terms, this level helps you reason about event time, out-of-order data, and durable consumers.
...
Complete or skim data_engineering:L06 first, then return here if a word feels unfamiliar.
...
- **Watermarks**: explain what this means before you try to automate it.
- **At-least-once tradeoffs**: identify where it appears in a table, notebook, model, system, or equation.
- **Stateful streams**: describe how it can mislead you if you ignore it.
...
Imagine a folder of daily log files. You need to know where each file comes from, what schema it claims to follow, and how stateful streams would appear when something breaks.
...
- I can say what `p066` / `rare_disease_signal_mining` is trying to teach.
- I can identify the input, output, and one hidden assumption.
- I can explain `Watermarks` using a small example.
- I can explain `At-least-once tradeoffs` using the project files or notebook.
- I can explain why `Stateful streams` matters.
- I can write one limitation without weakening the value of the project.

## Mission Anchor

- Project ID: `p066`
- Project slug: `rare_disease_signal_mining`
- Mission ID: `p066-de-l07-m01`
- Central notebook: [`notebooks/tracks/data_engineering/L07/data_engineering_l07_streaming_and_event_systems.ipynb`](../../../../../notebooks/tracks/data_engineering/L07/data_engineering_l07_streaming_and_event_systems.ipynb)
- Project notebook: `projects/rare_disease_signal_mining/notebooks/rare_disease_signal_mining.ipynb`
- Run command:

```bash
PYTHONPATH=src:. uv run python -m projects.rare_disease_signal_mining.src.pipeline
```
...
The next level is data_engineering:L08, where this idea becomes more technical.
